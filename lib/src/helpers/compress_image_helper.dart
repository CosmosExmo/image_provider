...
class CompressImageHelper {
  ...
  const CompressImageHelper(this.compressionOptions);
  ...
  Future<Uint8List> compressList(Uint8List list) async {
    try {
      var result = await FlutterImageCompress.compressWithList(
        list,
        minWidth: compressionOptions.minWidth,
        minHeight: compressionOptions.minHeight,
        quality: compressionOptions.quality,
        rotate: compressionOptions.rotate,
        inSampleSize: compressionOptions.inSampleSize,
        autoCorrectionAngle: compressionOptions.autoCorrectionAngle,
        format: compressionOptions.format,
        keepExif: compressionOptions.keepExif,
      );
      return result;
    } catch (_) {
      rethrow;
